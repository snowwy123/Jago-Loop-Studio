...
function paintMotionPreview(now){const previous=motionSampleActive;motionSampleActive=true;try{drawMotionPreview(now);}finally{motionSampleActive=previous;}}
function drawMotionPreview(now){if(!uiReady||currentPanel!=='motion'||Math.floor(now/80)===lastMotionPreview)return;lastMotionPreview=Math.floor(now/80);const previewTick=motionPaused?motionPauseTick:now/1000*project.boil;const c=$('motionPreviewCanvas'),cx=c.getContext('2d');cx.clearRect(0,0,c.width,c.height);if(motionDraft.type==='crawl'){drawStamp(cx,{kind:'stroke',tool:'stamp',color:'#dcff72',size:32,opacity:1,pressure:false,animate:true,mirror:false,seed:42,brush:{...stampSettings,tip:'chalk'},motion:copy(motionDraft),points:[{x:35,y:40,p:.5},{x:205,y:40,p:.5}]},previewTick);return;}cx.strokeStyle='#dcff72';cx.lineWidth=2.2;cx.lineCap='round';cx.beginPath();const m=copy(motionDraft);m.amount=Math.min(m.amount,20);m.wavelength=Math.min(m.wavelength,90);const b={x:35,y:24,w:170,h:30};for(let i=0;i<=40;i++){const p={x:35+i/40*170,y:40+Math.sin(i/40*Math.PI*2)*12},q=motionPoint(p,m,previewTick,b,3,i/40);i?cx.lineTo(q.x,q.y):cx.moveTo(q.x,q.y);}cx.stroke();if(m.type==='crawl'){cx.fillStyle='#b1a0ff';for(let i=0;i<25;i++){cx.globalAlpha=.3+.7*random(i+Math.floor(now/100));cx.fillRect(35+random(i*7)*170,24+random(i*19)*32,2,2);}cx.globalAlpha=1;}}
function paintBrushPreview(){
 ...
 const extent=160*(1+stampSettings.scatter)*1.6,w=Math.ceil(Math.max(240,extent*2.6)/cell)*cell,h=Math.ceil(Math.max(100,extent)/cell)*cell;
 ...
 $('brushPreviewLabel').textContent=brushSize+' px brush · '+Math.round(opacity*100)+'% opacity · '+(cell===1?'Smooth':'Pixel '+cell+' px')+' · preview scale stays fixed as Size changes';
}
...
function renderLibrary(){
 ...
 for(const kind of ['brushes','motions'])studioLibrary[kind].forEach((preset,index)=>{
  const row=document.createElement('div');row.className='preset-row';
  const apply=document.createElement('button');apply.textContent=(kind==='brushes'?'Use brush: ':'Use saved settings: ')+presetDisplayName(kind,preset);
  apply.onclick=async()=>{if(await useSavedPreset(kind,index))$('libraryDialog').close();};
  const remove=document.createElement('button');remove.textContent='×';remove.setAttribute('aria-label','Delete preset '+preset.name);
  remove.onclick=()=>deleteLibraryPresets(new Set([preset]));
  const select=document.createElement('input');select.type='checkbox';select.checked=librarySelection.has(preset);select.setAttribute('aria-label','Select '+preset.name+' for removal');select.onchange=()=>{if(select.checked)librarySelection.add(preset);else librarySelection.delete(preset);$('deleteSelectedPresets').disabled=!librarySelection.size;};
  row.append(select,apply,remove);list.append(row);
 });
 if(!list.children.length)list.textContent='No saved presets yet. Import a pack to get started.';
 $('deleteSelectedPresets').disabled=!librarySelection.size;$('undoPresetRemoval').disabled=!lastPresetRemoval;
...
function validateLibrary(v){
 ...
 const result={brushes:[],motions:[]};for(const p of v.motions){if(typeof p.name!=='string'||p.name.length>50)throw Error('Invalid preset name');result.motions.push({name:p.name,origin:['imported','saved'].includes(p.origin)?p.origin:undefined,motion:validateMotion(p.motion)});}for(const p of v.brushes){if(typeof p.name!=='string'||p.name.length>50||!finite(p.size,1,160)||!finite(p.opacity,0,1)||!/^#[0-9a-f]{6}$/i.test(p.colour))throw Error('Invalid brush preset');const brush=validateBrush(p.brush);if(!builtinTips.includes(brush.tip)){if(!p.tip||typeof p.tip.name!=='string'||p.tip.name.length>50||typeof p.tip.src!=='string'||!/^data:image\/png;base64,/.test(p.tip.src)||p.tip.src.length>2e6)throw Error('Custom brush data missing');}result.brushes.push({...p,origin:['imported','saved'].includes(p.origin)?p.origin:undefined,brush});}return result;
}
...
function initialiseStudio(){
 ...
 $('motionPreviewToggle').onchange=e=>{motionPreview=e.target.checked;if(motionPreview){motionPaused=false;lastTick=-1;}syncMotionUI();updateStatus();invalidate();};$('applyLayerMotion').onclick=applyMotionToLayer;$('applyStrokeMotion').onclick=applyMotionToStrokes;
 ...
 $('saveMotionPreset').onclick=()=>{const name=$('presetName').value.trim();if(!name){toast('Give this preset a name');return;}if(studioLibrary.motions.length>=30){toast('Delete a preset before adding another');return;}studioLibrary.motions.push({name,origin:'saved',motion:copy(motionDraft)});saveLibrary();renderLibrary();};
 $('saveBrushPreset').onclick=()=>{const name=$('presetName').value.trim();if(!name){toast('Give this preset a name');return;}if(studioLibrary.brushes.length>=30){toast('Delete a preset before adding another');return;}studioLibrary.brushes.push({name,origin:'saved',brush:copy(stampSettings),size:brushSize,opacity,colour,tip:project.brushTips?.[stampSettings.tip]?copy(project.brushTips[stampSettings.tip]):undefined});saveLibrary();renderLibrary();};
 $('exportPresets').onclick=()=>download(new Blob([JSON.stringify({format:'jago-presets',version:1,...studioLibrary})],{type:'application/json'}),'Jago-brush-and-motion-presets.jagopresets');$('importPresets').onclick=()=>$('presetInput').click();$('presetInput').onchange=importPresetLibrary;
