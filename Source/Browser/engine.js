...
function fresh(w=960,h=640){const id=uid();return{format:'jago-loop-studio',version:5,name:'Untitled loop',width:w,height:h,paper:'#fffdf9',transparent:false,renderStyle:'smooth',pixelSize:4,wiggle:3,boil:8,wiggleEnabled:true,fps:8,loop:'loop',layers:[{id,name:'Ink',visible:true,locked:false,opacity:1}],frames:[{id:uid(),hold:1,contents:{[id]:[]}}],current:0,activeLayer:id};}
...
function updateStatus(){$('toolStatus').textContent=`${toolDefs.find(t=>t[0]===tool)[1]} · ${brushSize} px`;$('layerStatus').textContent=activeLayer().name;$('activityStatus').textContent=playing?`Playing · frame ${playIndex+1} / ${project.frames.length}`:motionPaused?'Live motion paused':motionPreview?'Layer motion preview':project.wiggleEnabled&&(project.wiggle>0||projectHasMotion())?'Live motion on':'Still lines';}
function updateUI(){
...
function zoomAt(factor,x,y){const v=$('viewport').getBoundingClientRect(),old=zoom;zoom=clamp(zoom*factor,.08,project.canvasMode==='pixel'?40:5);if(x!==undefined){const cx=x-v.left-v.width/2,cy=y-v.top-v.height/2;pan.x=cx-(cx-pan.x)*zoom/old;pan.y=cy-(cy-pan.y)*zoom/old;}applyView();}
