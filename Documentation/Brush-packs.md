...
- **Use your drawing:** Draw a small mark in Jago Loop Studio. Use Rectangle selection or Lasso to select it on the active layer. Open Draw > Stamp brushes and choose **Create stamp from selection**. This captures a still image, without its animation.
- **Use an image file:** Open Draw > Stamp brushes and choose **Import stamp image**, then select a PNG. Use a transparent background to avoid a rectangular box around your stamp.

Choose your new stamp under **Stamp shape**. To keep an imported image's own colours, open Variation & pressure and turn on **Keep original image colours**. Leave it off to use your chosen drawing colour.
...
To load a pack, open **Stamp presets**, choose **Import library** and select the file. Its presets are added to your library. Click a saved stamp's name to use it.
